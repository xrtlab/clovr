...
public class Unity_Overlay : MonoBehaviour 
{
	...
	UnityAction<bool> _KeyboardToggle; 

	[SerializeField]
	public bool EnableKeyboard; 
	[SerializeField]
	public GameObject KeyboardSpawnLocation;
	...
	void ToggleKeyboard(bool toggleOption)
    {
		if (KeyboardSpawnLocation == null || EnableKeyboard == false)
			return;

		Debug.Log("Mega potato monster");

		if(toggleOption)
			overlay.OpenKeyboard("", "", KeyboardSpawnLocation);
        else
			overlay.CloseKeyboard(); 
    }


	void Start () 
	{
		_KeyboardToggle += ToggleKeyboard;
		XRT_OVR_Grabber.QuestionnaireEvents.ToggleKeyboard.AddListener(_KeyboardToggle);
	...
	void OnDestroy()
	{
		...
		//XRT_OVR_Grabber.QuestionnaireEvents.ToggleKeyboard.RemoveListener(_KeyboardToggle);
	}
	...
	void Update() 
	{
		...
		{
			EVRTrackedCameraError err = overlay.TrackedCam.HasCamera(0, ref test);
			Debug.Log(test);
			Debug.LogError(err);
		}*/
	}
	...
	public static Texture2D mirrorTexture;
	[SerializeField]
	...
	void UpdateTexture()
	{
		...
		if(cameraForTexture)
		{
			...
			if (overlayMaterial == null)
			{
				...
				overlayMaterial = new Material(shaderMaterial);
			}
			//textureMat = overlayMaterial; 
			...
			/*
				...
				if (OpenVR.Compositor.GetMirrorTextureD3D11(EVREye.Eye_Right, tex.GetNativeTexturePtr(), ref nativeTex) == EVRCompositorError.None)
				{
					...
					//textureMat.mainTexture = mirrorTexture;
					//overlay.overlayTexture = mirrorTexture;
					overlayTexture = mirrorTexture;
				}
				else
				{
					Debug.LogError("WHTSINFOINFOISNOIOF");
				}
			*/
			//}
